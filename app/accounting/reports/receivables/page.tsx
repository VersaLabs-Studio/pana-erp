// app/accounting/reports/receivables/page.tsx
// Obsidian ERP v4.0 — Receivables Report (Part 16)
//
// AR aging report with outstanding invoices, paid vs unpaid status,
...
function exportCsv(rows: ReceivableRow[]) {
  const header = [
    "Invoice",
    "Customer",
    "Invoice Date",
...
export default function ReceivablesReportPage() {
  ...
  const filterArgs = useMemo(() => {
    ...
    if (parties.length > 0) {
      base.push(["customer", "in", parties]);
    }
    return base;
  }, [statuses, parties]);

  const { data: invoices = [], isLoading } = useFrappeList<ReceivableRow>(
    "Sales Invoice",
    {
      fields: [
        "name",
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Outstanding customer invoices with aging analysis.
            </p>
          </div>
        </div>
        <Button
